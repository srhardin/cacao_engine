...
void BaseApplication::CreateGraphicsPipeline()
{
    ...
    // Classic transparency = SRC_ALPHA -> ONE_MINUS_SRC_ALPHA
    // Look at VkBlendFactor and VkBlendOp
...
VkShaderModule BaseApplication::CreateShaderModule(const std::vector<char>& code)
{
...
void BaseApplication::RecordCommandBuffer(VkCommandBuffer commandBuffer, uint32_t imageIndex)
{
    ...
    // Inline = commands run from primary command buffer
    // Secondary Command Buffers = commands run from secondary command buffers
